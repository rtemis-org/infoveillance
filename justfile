...
[doc("Lint package source for unused objects (variables/arguments)")]
lint:
    @just _msg "─── Linting {{ pkg }} source for unused objects... ───"
    {{ rscript }} -e "suppressMessages(pkgload::load_all('.', quiet = TRUE)); l <- lintr::lint_dir('R', linters = list(lintr::object_usage_linter())); print(l); if (length(l) > 0L) quit(status = 1L)"
    @just _msg "Done"
